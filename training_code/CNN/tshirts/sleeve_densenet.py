...
data_path = '/hdd_ext/hdd1/lhy0807/dataset/sleeve_2class/'
image_size = (224, 224)
...
plt.xlabel('epoch')
plt.ylabel('loss/acc')
plt.title('Training and Validation Loss_sleeve')

plt.show()

plt.savefig(os.path.join(data_path, 'densenet_freeze'))
...
model.compile(optimizer=optimizers.Adam(lr=0.0001),  # Very low learning rate
              loss='categorical_crossentropy',
              metrics=['acc'])
...
plt.savefig(os.path.join(data_path, 'densenet_unfreeze'))

model.save(os.path.join(data_path, 'models/transfer_densenet2.h5'))